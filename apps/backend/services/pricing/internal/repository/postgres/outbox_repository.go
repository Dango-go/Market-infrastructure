...
func (r *outboxRepository) Enqueue(ctx context.Context, env events.Envelope) error {
	if _, err := r.db.Exec(ctx, `INSERT INTO outbox (id, type, version, source, subject, correlation_id, occurred_at, data) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, env.ID, string(env.Type), env.Version, env.Source, env.Subject, env.CorrelationID, env.OccurredAt, env.Data); err != nil {
...
func (r *outboxRepository) MarkPublished(ctx context.Context, ids []uuid.UUID, publishedAt time.Time) error {
	...
	if _, err := r.db.Exec(ctx, `UPDATE outbox SET published_at = $2 WHERE id = ANY($1::uuid[])`, ids, publishedAt); err != nil {
